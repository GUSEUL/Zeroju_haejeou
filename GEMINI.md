...
1. **Delay Penalty**: $r_{\text{delay}} = -(w_{\text{task}} \cdot \text{TotalDelay} \cdot 2.0)$
   ...
   - **TotalDelay** includes transmission delay and computational delay (based on local or neighbor queue size).
...
- **`mdc_gym_env.py`**: The primary Gymnasium environment.
    - **Neighbor Queues**: Tracks 6 individual neighbor queues. Offloading increases the target neighbor's queue.
    - **Dynamic Delay**: Offloading delay is calculated as: $Delay_{comp} = \frac{NeighborQueue}{10.0} + 0.05$.
...
- **`visualize_network.py`**:
    - **Role**: High-fidelity Pygame simulation.
    - **Multi-Node Monitoring**: Real-time queue gauges for both Main MDC and all 6 Neighbor MDCs.
    - **Visual Cues**: 
        - **Orange**: Local processing (Internal pulse at center).
        - **Blue**: Offloading (Packet moves to specific neighbor N1-N6).
        - **Yellow Highlight**: Indicates which neighbor is currently receiving a task.
        - **Red Flash**: Task dropped due to intentional action or overflow.

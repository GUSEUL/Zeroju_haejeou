...
def run_tsne(lambda_val):
    ...
    if res.returncode != 0:
        ...
        return False
    print(f"Finished t-SNE: Lambda={lambda_val}, Reward={reward_type} in {time.time() - start:.2f} seconds")
    return True

if __name__ == "__main__":
    t_start = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(run_tsne, l) for l in lambdas]
        for f in concurrent.futures.as_completed(futures):
            f.result()
    print(f"All improved t-SNE plots generated in {time.time() - t_start:.2f} seconds.")

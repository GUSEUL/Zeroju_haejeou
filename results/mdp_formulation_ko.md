...
1. **상대적으로 높은 연산 능력 (High Computing Capability)**:
   - 단말기의 로컬 처리 속도 계수인 $\text{ServiceRate} = 2$와 달리, 이웃 에지 노드들은 각각 연산 능력 계수(분모)가 **$8.0$**으로 설정되어 있습니다. 
   - 따라서 이웃 노드 $i$로 태스크가 오프로딩되었을 때의 실제 계산 시간은 $\text{Delay}_{\text{comp}} = \frac{Q_{n_i} + 1}{8.0} + 0.05$로 산출되어, 단말기 로컬 처리($\text{Delay}_{\text{comp}} = \frac{Q_{\text{local}} + 1}{2 \cdot \text{ServiceRate}} = \frac{Q_{\text{local}} + 1}{4.0}$)에 비해 큐당 대기 시간이 매우 짧습니다.
...
3. **시간에 따른 동적 태스크 처리율 (Stochastic Service Rates)**:
   - 매 타임스텝마다 이웃 노드 1, 2의 대기열 크기($Q_{n_1}, Q_{n_2}$)는 에지 노드 자체의 태스크 처리로 인해 **$1$ 또는 $2$개만큼씩 무작위로 감소**합니다.
   - 이때 감소하는 처리율의 전이 확률은 다음과 같이 동일한 확률($50\%$)을 지닙니다.
     $$P(\text{처리율} = 1) = 0.5, \quad P(\text{처리율} = 2) = 0.5 \quad (\text{기대 처리율 } \mathbb{E}[\text{ServiceRate}_{\text{neighbor}}] = 1.5)$$

4. **대칭적 물리 특성과 부하 분산(Load Balancing) 필요성**:
   - 수학적 모델링 상 두 에지 노드는 동일한 연산 능력 계수($8.0$)와 동일한 확률적 처리 감쇠($1.5$)를 가지며 대칭적입니다.
   - 하지만 에이전트 관점에서는 이웃 노드 1의 대기열 크기($Q_{n_1}$)와 이웃 노드 2의 대기열 크기($Q_{n_2}$)가 독립적인 차원으로 존재합니다. 따라서 에이전트는 무작위로 축적되는 두 큐의 현재 상태를 실시간으로 비교하여 더 한산한 노드로 태스크를 보내는 **동적 부하 분산(Load Balancing) 정책**을 스스로 학습하게 됩니다.
...
$$\text{Cost}_{\text{delay, energy}} = w_{\text{task}} \cdot w_{\text{delay}} \cdot \text{NormDelay} + (1 - w_{\text{delay}}) \cdot \text{NormEnergy}$$

* **$w_{\text{delay}} = 0.6$**: 전체 비용 계산 중 지연 시간에 적용되는 가중치 비율
* **$\text{NormDelay} = \min\left(1.0, \frac{\text{TotalDelay}}{\text{MaxDelay}}\right)$**: 최대 5.0초를 기준으로 지연 시간을 $0 \sim 1$ 사이로 정규화 ($\text{MaxDelay} = 5.0$)
...
각 행동 $a$에 대해 소요되는 전송 지연($\text{Delay}_{\text{trans}}$), 컴퓨팅 연산 지연($\text{Delay}_{\text{comp}}$), 에너지($\text{EnergyConsumed}$)는 다음과 같습니다.
...
### 3.3 대기열 혼잡 패널티 ($\text{Penalty}_{\text{queue}}$ & $\text{Penalty}_{\text{neighbor}}$)
...
시스템의 다음 상태 $S_{\text{next}}$로의 전이는 이전 상태 $S$와 에이전트의 행동 $a$에 의해 결정됩니다. 본 MDP 모델은 다음 상태를 구성하는 개별 상태 변수들이 조건부 독립(Conditional Independence)이라는 가정 하에 설계되었습니다.
...
* **$\text{Comm} = 0$ (불량)인 경우**:
  $$P(Comm_{\text{next}} = 0 \mid \text{Comm} = 0) = 0.9$$
  $$P(Comm_{\text{next}} = 1 \mid \text{Comm} = 0) = 0.1$$
  $$P(Comm_{\text{next}} = 2 \mid \text{Comm} = 0) = 0.0$$

* **$\text{Comm} = 1$ (보통)인 경우**:
  $$P(Comm_{\text{next}} = 0 \mid \text{Comm} = 1) = 0.1$$
  $$P(Comm_{\text{next}} = 1 \mid \text{Comm} = 1) = 0.8$$
  $$P(Comm_{\text{next}} = 2 \mid \text{Comm} = 1) = 0.1$$

* **$\text{Comm} = 2$ (좋음)인 경우**:
  $$P(Comm_{\text{next}} = 0 \mid \text{Comm} = 2) = 0.0$$
...
* **학습 시 - 무작위 시작 (Exploring Starts - `random_start=True`)**:
  - 학습 시에는 에이전트가 상태 공간 전체를 고르게 방문할 수 있도록 매 에피소드 시작 시 상태 변수들을 무작위로 초기화합니다.
    $$Q_{\text{local}} \sim \text{Uniform}(0, 4), \quad Q_{n_1}, Q_{n_2} \sim \text{Uniform}(0, 10), \quad \text{Comm} \sim \text{Uniform}(0, 2), \quad \text{Task} \sim \text{Uniform}(0, 1)$$
...
* **평가 시 - 고정 시작 (`random_start=False`)**:
  - 학습이 완료된 알고리즘의 절대 성능을 일관성 있게 검증하기 위해, 평가 모드에서는 항상 동일한 시작 상태에서 출발합니다.
    $$S_{\text{start}} = [\text{Task}=0, \text{Comm}=1, Q_{\text{local}}=0, Q_{n_1}=0, Q_{n_2}=0]$$
...
* **수정**: **100,000 에피소드** 학습으로 연장 및 감쇠 스케줄러 완화 (총 에피소드의 **$60\%$** 시점인 $\text{episodes} \times 0.6 = 60,000$ 에피소드까지 서서히 감쇠)
  - 탐험 감쇠는 `decay = np.exp(np.log(eps_min)/(episodes * 0.6))` 공식을 활용하여 탐험율 $\epsilon$이 $100,000 \times 0.6 = 60,000$ 에피소드 시점까지 점진적으로 감소하도록 설계되었습니다.
  - 학습 수렴 횟수를 총 20배 늘려 희소 상태 공간(State Sparsity)의 충분한 방문 빈도와 완벽한 Q-table 수렴을 보장함.
...
* **최근 검증 일시**: 2026-05-25
* **검증 대상 코드**: `mdc_mdp_env.py`, `build_mdp_model.py`, `train_all_mdp.py`
* **검증 내용**: 
  - 단말기 및 이웃 노드의 계산/전송 지연 산출식 분모와 가중치들의 정밀성 검증 완료.
  - Cliff 설정의 보상 함수는 Standard 설정의 QoS 가중치 및 대기열 패널티($\beta_{\text{local}}$, $\beta_{\text{neighbor}}$)를 동일하게 물려받고, 대기(Pending) 패널티($\gamma$)에 대해서만 강력한 $1000.0$ 상수를 곱해 보완 적용하는 방식으로 싱크를 일치시킴.
  - 학습 중 기록되는 보상이 기존의 단순 누적 합산에서 실제 평가 메트릭과 동일한 할인 누적 보상(Discounted Return, $\sum \gamma^t r$)을 누적하는 방식으로 개선되어 스케일 차이가 완벽히 해결됨을 검증 완료.

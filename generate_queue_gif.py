...
# Load Q-tables
q_std = np.loadtxt("results/L_1.5_E_30000/standard/q_table_ql.csv", delimiter=",")
q_imp = np.loadtxt("results/L_1.5_E_20000/improved/q_table_ql.csv", delimiter=",")
...
def simulate_step(q_table, state, arrival, task_type, comm_next):
...
gif_vis_path = os.path.join(vis_dir, "queue_simulation.gif")
gif_res_path = os.path.join(res_dir, "queue_simulation.gif")

# Save as GIF
anim.save(gif_vis_path, writer='pillow', fps=1.5)
print(f"Saved animation to: {gif_vis_path}")

# Copy to results directory
import shutil
shutil.copyfile(gif_vis_path, gif_res_path)
print(f"Copied animation to: {gif_res_path}")

# Copy to artifacts directory
artifacts_dir = r"C:\Users\sbeen\.gemini\antigravity-cli\brain\a77332d9-2947-4557-b570-286414b9617e"
os.makedirs(artifacts_dir, exist_ok=True)
gif_art_path = os.path.join(artifacts_dir, "queue_simulation.gif")
shutil.copyfile(gif_vis_path, gif_art_path)
print(f"Copied animation to artifacts: {gif_art_path}")
